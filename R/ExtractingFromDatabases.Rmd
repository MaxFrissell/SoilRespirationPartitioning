...
read.csv(here("Data", "srdb-data.csv")) %>%
  ...
  subset(!is.na(Site_ID) & !is.na(Latitude) & !is.na(Longitude) & !is.na(Leaf_habit) & Leaf_habit != "" & !is.na(Rs_annual) & !is.na(Rh_annual) & RC_annual >= 0 & RC_annual <= 1) -> srdb
```
...
## Note: the .TIF files used are not included in the repo and need to be downloaded from the "nasoudzilovskaia/Soudzilovskaia_NatureComm_MycoMaps" GitHub repo
...
EM_precent = raster::extract(em, allData[, c(3, 2)])
ER_precent = raster::extract(er, allData[, c(3, 2)])
NM_precent = raster::extract(nm, allData[, c(3, 2)])
allData = cbind(allData, AM_percent, EM_precent, ER_precent, NM_precent)
...
ggplot(allData, aes(x = Longitude, y = Latitude, col = EM_precent)) + 
  geom_point(size = 1.5) + 
  lims(x = c(-180, 180), y = c(-50, 80), color = c(0, 100)) + 
  theme_minimal()

ggplot(allData, aes(x = Longitude, y = Latitude, col = ER_precent)) + 
  geom_point(size = 1.5) + 
  lims(x = c(-180, 180), y = c(-50, 80), color = c(0, 100)) + 
  theme_minimal()

ggplot(allData, aes(x = Longitude, y = Latitude, col = NM_precent)) + 
  geom_point(size = 1.5) + 
  lims(x = c(-180, 180), y = c(-50, 80), color = c(0, 100)) + 
  theme_minimal()
```
...
ggplot(allData, aes(x = MAT_WC, y = EM_precent)) + 
  geom_point(size = 1.5, color = "chartreuse3") + 
  theme_minimal()

ggplot(allData, aes(x = MAT_WC, y = ER_precent)) + 
  geom_point(size = 1.5, color = "chartreuse3") + 
  theme_minimal()

ggplot(allData, aes(x = MAT_WC, y = NM_precent)) + 
  geom_point(size = 1.5, color = "chartreuse3") + 
  theme_minimal()
```
...
``` {r Myco&MAP}
## Plot the ER myco precentage vs MAP
## ER is the only one that showed any visible correlation

ggplot(allData, aes(x = MAP_WC, y = ER_precent)) + 
  geom_point(size = 1.5, color = "chartreuse3") + 
  theme_minimal()
```
...
I searched for some correlations between the biomass values and some of the other data, like MAT, MAP, root contribution, and Rs, but I didn't really find anything.