...
Here are some graphs showing various relationships between N deposition and other variables.
...
## Extract data on bulk density, clay content, soil type, and organic carbon stock from SoilGrids
## Note: These files are not in the repository and were downloaded through the ISRIC file service at https://files.isric.org/public/sg250m/ between 7/24/20 and 7/30/20
## This tool can be a struggle to use, you can email me at maxfrissell@gmail.com and I can try to send it to you if you can't get it to work

# Convert lat, long degree coordinates into meter coordinates
cbind(allData$Longitude, allData$Latitude) %>%
  latlong2grid() %>%
  round() * 1000 %>%
  round_any(250) -> coordsInM

# Extract bulk density values for the coordinates
raster(here::here("Data", "bdod_5-15cm_Q0.5.vrt")) %>%
  raster::extract(coordsInM) -> Bulk_density

# Add this data to the main dataframe
allData = cbind(allData, coordsInM, Bulk_density)
summary(allData)
allData
```