...
read.csv(here::here("Data", "srdb-data.csv")) %>%
  ...
  subset(!is.na(Site_ID) & !is.na(Latitude) & !is.na(Longitude) & !is.na(Leaf_habit) & !is.na(Rs_annual) & !is.na(Rh_annual) & RC_annual > 0 & RC_annual < 1 & !is.na(RC_annual) & Manipulation == "None") -> srdb
```
...
# Add the absolue value of latitude to the dataset
...
ggplot(trainWithP, aes(x = RC_annual, y = prediction2)) +
  lims(y = c(0, 1)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", size = 1, color = "red")
...
ggplot(testWithP, aes(x = RC_annual, y = prediction2)) +
  lims(y = c(0, 1)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", size = 1, color = "red")
